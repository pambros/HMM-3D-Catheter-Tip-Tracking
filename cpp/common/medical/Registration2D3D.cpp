...
Registration2D3D::Registration2D3D(const qString &_confileFile, const qString &_dataFolder) : m_Parameters(NULL)
			...
			, m_DataList(_dataFolder)
			, m_CurrentFrame(0){
	m_Parameters = Q_NEW Parameters(_confileFile);

	if(m_Parameters->m_Method == METHOD_SHAPE_SIMILARITY){
		m_Object2D3DRegistration = static_cast<Object2D3D*>(Q_NEW ShapeSimilarity2D3D(static_cast<OPTIMIZER_ENUM>(m_Parameters->m_Optimizer), _confileFile, m_DataList.m_Vessel3draFileName, m_DataList.m_Info3draFileName));
	}
...
void Registration2D3D::Register(void){
	...
	qs32 ret = m_Object2D3DRegistration->Do2D3DRegistration(catheter, infoFluoro);
}
